  ...
  stages {
    ...
    stage('Check Pact Verifications') {
      steps {
        ...
        dir('pact/bin') {
          sh "./pact-broker can-i-deploy --retry-while-unknown=12 --retry-interval=10 -a messaging-app -b http://pact_broker -e ${GIT_COMMIT} --to prod"
          sh "./pact-broker can-i-deploy --retry-while-unknown=12 --retry-interval=10 -a messaging-app2 -b http://pact_broker -e ${GIT_COMMIT} --to prod"
          sh "./pact-broker can-i-deploy --retry-while-unknown=12 --retry-interval=10 -a messaging-app3 -b http://pact_broker -e ${GIT_COMMIT} --to prod"
        }
      }
    }
    stage('Deploy') {
